describe("cacApp.country.service", function () {
  ...
  var capitalPopResolve = Math.random();
  var neighborsResolve = Math.random();
    ...
    module(function($provide){
      GeonamesSvcMock = {
        ...
        capitalPopulation: function(){
          return Promise.resolve(capitalPopResolve);
        },
        neighbors: function(){
          return Promise.resolve(neighborsResolve);
        }
      };
      ...
      inject(function (CountrySvc) {
        CountrySvc.getCountry().then(function(country){
          CountrySvc.addCapitalPopulationTo(country).then(function(country){
            expect(country.capitalPopulation).toBe(capitalPopResolve);
      ...
      inject(function (CountrySvc) {
        CountrySvc.getCountry().then(function(country){
          CountrySvc.addNeighborsTo(country).then(function(country){
            expect(country.neighbors).toEqual(neighborsResolve);
